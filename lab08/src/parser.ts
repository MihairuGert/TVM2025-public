...
export const getFunnyAst = {
    ...
    Assignment_tuple(ltargertlist: any, equals, rexpr: any, semi) {
        ...
        return { type: "assign", left: targets, right: expr } as ast.Ass;
    },
   
    Assignment_simple(ltargert: any, equals, rexpr: any, semi) {
        const target = ltargert.parse();
        const expr = rexpr.parse();
        return { type: "assign", left: [target], right: [expr] } as ast.Ass;
    },

    Assignment_array(array: any, equals, rexpr: any, semi) {
        return { type: "assign", left: [array.parse()], right: [rexpr.parse()] } as ast.Ass;
    },
    ...
    Block(left_brace, statements: any, right_brace) {
        const stmts_list = statements.children.length > 0
            ...
            : [];
        return { type: "blk", stmts: stmts_list } as ast.Blk;
    },
...
function getLocation(ctx: any): ast.Location | undefined {
    if (ctx.interval && ctx.source) {
        return {
            start: {
                line: ctx.source.lineNumAt(ctx.interval.startIdx),
                column: ctx.source.colNumAt(ctx.interval.startIdx)
            },
            end: {
                line: ctx.source.lineNumAt(ctx.interval.endIdx),
                column: ctx.source.colNumAt(ctx.interval.endIdx)
            }
        };
    }
    return undefined;
